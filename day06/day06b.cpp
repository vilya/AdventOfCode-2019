...
int main(int argc, char** argv)
{
  ...
  std::vector<size_t> youToRoot;
  size_t i = ids["YOU"];
//  youToRoot.push_back(i);
  do {
    i = orbits[i];
    youToRoot.push_back(i);
  } while (i != 0);

  std::vector<size_t> sanToRoot;
  i = ids["SAN"];
  do {
    i = orbits[i];
    sanToRoot.push_back(i);
  } while (i != 0);

  while (!youToRoot.empty() && !sanToRoot.empty() && youToRoot.back() == sanToRoot.back()) {
    youToRoot.pop_back();
    sanToRoot.pop_back();
  }

  uint64_t transfers = youToRoot.size() + sanToRoot.size();
