...
int main(int argc, char** argv)
{
  ...
  const int64_t initialOre = 1000000000000ll;
  const uint64_t costOfOne = data.cost_of_fuel(1);

  // Estimate the range that the solution lies in, then do a binary search for
  // it within that range.
  uint64_t lo = initialOre / costOfOne;
  uint64_t hi = lo * 2;
  while (lo + 1 < hi) {
    uint64_t mid = (lo + hi) / 2;
    uint64_t cost = data.cost_of_fuel(mid);
    if (cost > initialOre) {
      hi = mid;
    }
    else if (cost < initialOre) {
      lo = mid;
    }
    else {
      printf("exactly %llu fuel produced\n", mid);
      return 0;
    }
  }
  uint64_t n = hi;
  if (data.cost_of_fuel(n) > initialOre) {
    --n;
  }
  printf("%llu fuel produced\n", n);
  return 0;
}
